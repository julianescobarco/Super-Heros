...
resource "aws_lb_listener_rule" "listener_rule" {
  count = length(var.http_rules) > 0 ? length(var.http_rules) : 0
  ...
  dynamic "action" {
    for_each = [
      for action_rule in var.http_rules[count.index].actions :
      action_rule
  ...
  dynamic "action" {
    ...
    content {
      type             = action.value["type"]
      target_group_arn = element(var.target_group, lookup(action.value, "target_group_index", count.index))
    }
  }
  ...
  dynamic "condition" {
    for_each = [
      for condition_rule in var.http_rules[count.index].conditions :
      condition_rule
...
output "id" { value = aws_lb_listener_rule.listener_rule.*.id }
output "arn" { value = aws_lb_listener_rule.listener_rule.*.arn }